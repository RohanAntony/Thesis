...
const fetchData = async (tickertag, symbol, name) => {
  ...
  const url = `https://www.tickertape.in/stocks/${tickertag}/events?checklist=basic&type=corpActions`
  await page.goto(url, {
  ...
  const rowClasses = [
    { class: ".event-table-title", name: 'title'},
    // { class: ".table-col-title .desktop--only .desktop--only span span span", name: 'title'},
    { class: ".table-col-date .event-table-data span", name: 'ratio'},
    { class: ".table-col-info .event-table-data > span", name: 'exchangeDate'}
  ];
  ...
  const screenshotPath = `./images/corpActions/${symbol}.png`
